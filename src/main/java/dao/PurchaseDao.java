...
//	        entityManager.createQuery("UPDATE User x SET x.name=:username WHERE x.id =:userid1").setParameter("username", username).executeUpdate();
    ...
    @Transactional
	public void save(String medicine, String company, Integer price, Integer qty) {
		EntityManager em=entityManagerProvider.get();
//		em.getTransaction().begin();
		Purchase purchase=new Purchase();
		purchase.setMedicineName(medicine);
		purchase.setMedicineCompany(company);
...
//		em.getTransaction().commit();
		...
		stock.setMedicineCompany(company);
		stock.setMedicineName(medicine);
